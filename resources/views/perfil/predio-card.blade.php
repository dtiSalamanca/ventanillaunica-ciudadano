...
<div class="predio-card" data-predio-id="{{ $predio->id_predio }}" data-estatus-predio="{{ $estatusPredio }}">
    <button class="predio-card__header" type="button" aria-expanded="{{ $tieneErrorCorreccion ? 'true' : 'false' }}">
        <div class="predio-card__info">
            ...
            <span class="predio-card__textos">
                ...
                <span class="predio-card__resumen">{{ $completadosPredio }} de {{ $totalDocumentosPredio }} documentos
                    cargados</span>
            </span>
        </div>
        <div class="predio-card__acciones-header">
            @if ($estatusPredio === 2)
                <span class="badge-estatus badge-estatus--aprobado predio-card__badge"><i
                        class="fas fa-circle-check me-1"></i>Aprobado</span>
            @elseif ($estatusPredio === 1)
                <span class="badge-estatus badge-estatus--revision predio-card__badge"><i
                        class="fas fa-hourglass-half me-1"></i>En revisión</span>
            @else
                <span class="badge-estatus badge-estatus--rechazado predio-card__badge"><i
    ...
    <div class="predio-card__body" @if (!$tieneErrorCorreccion) hidden @endif>
        @if ($estatusPredio === 0)
            <form action="{{ route('actualizarPredio', $predio->id_predio) }}" method="POST"
                class="form-agregar-predio form-corregir-predio">
                @csrf
                <div class="form-agregar-predio__campo">
                    <label for="clave_predio_{{ $predio->id_predio }}">Corregir cuenta predial</label>
                    <input type="text" name="clave_predio_{{ $predio->id_predio }}"
                        id="clave_predio_{{ $predio->id_predio }}"
                        value="{{ old('clave_predio_' . $predio->id_predio, $predio->clave_predio) }}"
                        placeholder="Ej. 123-456-789-000" maxlength="255" required>
        ...
        @if ($estatusPredio !== 2)
            <form action="{{ route('eliminarPredio', $predio->id_predio) }}" method="POST"
        ...
        @if ($catalogoPredios->isEmpty())
            <p class="mensaje-vacio"><i class="fas fa-circle-info me-1"></i>No hay documentos de predios disponibles en
                este momento.</p>
        @else
            @if ($estatusPredio !== 2)
                <p class="mensaje-vacio">
                    ...
                    @if ($estatusPredio === 0)
                        Este predio fue rechazado. Corrige y reenvía la clave para que sea validado antes de cargar sus
                        documentos.
                    @elseif ($estatusPredio === 1)
            ...
            <div class="documentos-lista">
                @foreach ($catalogoPredios as $documento)
                    @php
                        ...
                        // La vigencia corre desde la aprobación (respaldo: fecha de creación).
                        // Con vigencia 0 el documento no expira.
                        $fechaVencimiento =
                            $cargado && $documento->vigencia_meses > 0
                                ? ($cargado->fecha_aprobacion ?? $cargado->created_at)
                                    ->copy()
                    ...
                    <div class="documento-card {{ $cargado ? 'documento-card--cargado' : '' }}"
                        data-estatus="{{ $estatusDoc }}" data-catalogo-id="{{ $documento->id_documento_predio }}"
                            ...
                            <div class="documento-card__meta">
                                ...
                                @if ($cargado)
                                    <span class="documento-card__fecha"><i
                                        ...
                                        {{ $cargado->created_at->format('d/m/Y') }}</span>
                                    @if ($estatus === 2)
                        ...
                        <div class="documento-card__acciones">
                            @if ($cargado)
                                ...
                                <a href="{{ route('descargarDocumentoPredio', $cargado->id_documento_predio) }}"
                                    class="btn-accion btn-accion--ver" target="_blank" title="Ver documento">
                                    <i class="fas fa-eye"></i>
                                </a>
                                @if (($estatus === 0 || $documentoVencido) && $estatusPredio === 2)
                                    <form
                                        action="{{ route('subirDocumentoPredio', [$predio->id_predio, $documento->id_documento_predio]) }}"
                                        method="POST" enctype="multipart/form-data" class="form-subir-inline">
                                        @csrf
                                        <input type="file" name="archivo" class="input-archivo-oculto" accept=".pdf"
